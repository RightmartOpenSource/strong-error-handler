...
module.exports = function buildResponseData(err, options = {}) {
  ...
  var isSummarize = options.summarize;

  if (Array.isArray(err) && isDebugMode) {
    err = isSummarize ? serializeAndSummarizeArrayOfErrors(err) : serializeArrayOfErrors(err);
  }
...
function serializeAndSummarizeArrayOfErrors(errors) {
  let summError;

  errors.forEach((error) => {
    if (!summError || summError.statusCode < error.statusCode) { 
      summError = error; 
    } else if (summError.statusCode === error.statusCode) {
      summError = _.mergeWith(summError, error, (objValue, srcValue) => {
